...
const OrderFeedback = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!feedbackInit) {
      ...
      updateFeedback(
        { formData, id: feedbackInit.id.toString() },
        {
          onSuccess: () => {
            refetch();
                      ...
                      for (let i = 0; i < feedbackInit?.images?.length; i++) {
                        if (feedbackInit?.images[i].url === url) {
                          setDeleteImages((prev) => [
                            ...prev,
                            feedbackInit?.images[i].public_id,
                          ]);
                        }
                      }
                    }
          ...
          {feedbackInit && (
            <div className="flex justify-end">
              <button
                ...
                disabled={isPending}
                className="inline-flex cursor-pointer bg-red-500 justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm 
            hover:bg-red-600 focus:ring-offset-2 disabled:opacity-50"
              >
                {isPending ? "Đang xử lý..." : "Xóa phản hồi"}
              </button>
            </div>
          )}
          <div className="flex justify-end">
