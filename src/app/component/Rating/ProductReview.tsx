...
import { useDestroyReview } from "hooks/useDestroyReview";
...
const ProductReview = ({
  ...
  const [hoveredRating, setHoveredRating] = useState(
    dataReview ? dataReview.rating : 0
  );
  ...
  const handleEditInDrawer = (review: Review) => {
  ...
  const { mutate: destroyReview } = useDestroyReview();
  ...
  const handleDelete = (reviewId: number) => {
    destroyReview(
      ...
      {
        onSuccess: () => {
          setReviewsData((prev) => prev.filter((item) => item.id !== reviewId));
      ...
      {activeTab === "reviews" && reviewsData.length > 0 && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <RatingSummaryCard title="Đánh giá xếp hạng có 5 sao từ người dùng" />
            <RatingSummaryCard
      ...
      {activeTab === "reviews" && reviewsData.length === 0 && (
        <div className="bg-white rounded-lg p-6 shadow-sm">
          ...
          <p className="text-gray-600">
            Hãy là người đầu tiên viết đánh giá cho sản phẩm này!
          </p>
          <button
